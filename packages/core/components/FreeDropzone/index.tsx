...
const DropZoneEdit = forwardRef<HTMLDivElement, DropZoneProps>(
  function DropZoneEditInternal(
    ...
    const layout: Layout[] = contentWithPreview.map((item, i) => ({
      i: item.props.id,
      x: (i % 6) * 2,
      y: Math.floor(i / 6) * 2,
      w: 2,
      ...
      >
        ...
        >
          {contentWithPreview.map((item, i) => {
            ...
            return (
              <div key={componentId} data-grid={layout[i]}>
                <DropZoneProvider
...
const DropZoneRender = forwardRef<HTMLDivElement, DropZoneProps>(
  function DropZoneRenderInternal({ className, style, zone }, ref) {
    ...
    const layout: Layout[] = content.map((item, i) => ({
      i: item.props.id,
      x: (i % 6) * 2,
      y: Math.floor(i / 6) * 2,
      w: 2,
      ...
      <div className={className} style={style} ref={ref}>
        ...
        >
          {content.map((item, i) => {
            ...
            if (Component) {
              return (
                <div key={item.props.id} data-grid={layout[i]}>
                  <DropZoneProvider
                    key={item.props.id}
                    value={{
                      data,
